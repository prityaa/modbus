...
int main(int argc, char *argv[])
{
	...
	uint slaveid = 0, functionid = 3, startreg = 0, numofreg = 0;
    char CurrTime[20];
    ...
    {	
		...
		else
		{
			...
			GetTime(CurrTime,2,20);
			if (fs != NULL)
			{
				fprintf(fs,"[%s],",CurrTime);
				FilePrintHex(command,8,fs);
				fprintf(fs,"[%d],",ret);
			}	
			tx++;
		}
